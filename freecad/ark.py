...
def alle(doc, udmappe):
    ...
    tegn.update(kasse(doc, "Nagleraekke_venstre", "NAGLERAEKKE"))
...
def gavl_top(doc):
    """Lodret snit gennem gavltoppen: gavlregel, hjoernespaer, nagleraekke."""
    s = doc.Spreadsheet
    ...
    for navn, pts in stk:
        t.polygon(pts)
    t.maal_vandret(-float(s.nagle_b), 0, top + rh * .55, hjaelp_fra=top)
    t.maal_vandret(0, rb, top + rh * 1.15, hjaelp_fra=top)
    t.maal_vandret(0, rh, top - rh * 2.2, hjaelp_fra=top - rh * 1.9)
    t.note(rh * 1.9, top + rh * .30, "Nagleraekke - fastgoerelse for "
           "gavlbeklaedningens oeverste kant", -float(s.nagle_b) / 2, top)
    ...
    return {"gavl_top": _ark(
        t, "DETALJE: gavltop",
        "Nagleraekke %.0f mm uden paa hjoernespaeret %.0f mm"
        % (s.nagle_b, rb))}
